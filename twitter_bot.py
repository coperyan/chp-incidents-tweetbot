...
def get_twitter_api():
    creds = get_creds()
    api = twitter.Api(
        consumer_key=creds["twitterApiKey"],
...
def get_incident_format(incident):
    ...
    if (
        len(incident["incident_location_description"]) > 0
        and incident["incident_location_description"].lower()
        != incident["incident_location"].lower()
    ):
        tweet_text = "{}{}\n".format(
            tweet_text, incident["incident_location_description"]
        )
    if (
        len(incident["incident_area"]) > 0
        and incident["incident_area"] != incident["chp_center"]
    ):
        tweet_text = "{}{}".format(tweet_text, incident["incident_area"])
    # tweet_text = '{}{} CHP'.format(tweet_text,incident['chp_center'])
    tweet_text = "{}\n\n#CHP #CHP{}".format(
        tweet_text, incident["chp_center"].replace(" ", "")
    )
    if (
        len(incident["incident_area"]) > 0
        and incident["incident_area"] != incident["chp_center"]
        ...
        tweet_text = "{} #{}".format(
            tweet_text, incident["incident_area"].replace(" ", "")
        )
    return tweet_text
